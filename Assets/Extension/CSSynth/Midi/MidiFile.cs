...
namespace CSSynth.Midi {
    public class MidiFile {
        ...
        public int combinedTrackIdx;   // to know where the track is combined
        //--Public Properties
        ...
        public MidiFile(byte[] byteArray, Action<string> onError) {
        ...
        // Fix Jan3.3: Exception: Not a valid midi file
        // https://developer.cloud.unity3d.com/diagnostics/orgs/amanotespteltd/projects/f1e18c71-5fa3-4d32-841a-034a99577514/crashes/5d1970f67ec40eaba3857397a33125c9/
        public MidiFile(string filename, Action<string> onError) {
            this.onError = onError;
            Stream midiStream = null;
            try {
                //DeadNote
        ...
        public List<MidiEvent> getAllMidiEventsofType(MidiHelper.MidiChannelEvent eventChannelType, MidiHelper.MidiMetaEvent eventMetaType = MidiHelper.MidiMetaEvent.None, bool isRequireOriginal = false)
        {
            ...
            {
                if (tracks == null) return matchList;

                for (int x = 0; x < tracks.Length; x++)
                {
                    var track = tracks[x];
                    if (track == null) continue;
                    if (track.midiEvents == null) continue;

                    for (int i = 0; i < track.midiEvents.Length; i++)
                    {
                        var e = track.midiEvents[i];
                        if (e == null) continue;
                        if ((eventMetaType == MidiHelper.MidiMetaEvent.None
                             || e.midiMetaEvent == eventMetaType)
                            && e.midiChannelEvent == eventChannelType)
                        {
                            matchList.Add(e);
                        }
                    }
            ...
            {
                var midiEvents = combinedTrack.midiEvents;
                if (midiEvents == null) return matchList;
        ...
        // no real function here. Just to ap
        public void SetCombineTrack(MidiTrack combined)
        {
            combinedTrack = combined;
        }

        public void CombineTracks() {
            // Fix null Non-fatal Exception: NullReferenceException
            // https://console.firebase.google.com/project/new-magic-tiles/crashlytics/app/ios:com.amanotes.magictiles/issues/c80298c4fcf94f4fd8ca3e4228638436?time=last-seven-days&sessionId=4c984b9c76e3403b85c3f25e1ad57cb3_DNE_2_v2
            ...
            combinedTrackIdx = -1;
            const string TrackViewName = "FL Keys";

            // find the track with view name
            for (int i = 0; i < tracks.Length; i++)
            {
                if (tracks[i].trackName == null)
                    continue;

                if (tracks[i].trackName.Contains(TrackViewName))
                {
                    combinedTrackIdx = i;
                    break;
                }
            }
            // if track with name not exist, just combine every thing
            if (combinedTrackIdx == -1)
                combinedTrackIdx = tracks.Length - 1;
            //Loop to get track info
            for (int x = 0; x < combinedTrackIdx+1; x++) {
                if (tracks[x] == null) continue;

                total_eventCount = (tracks[x].midiEvents == null) ? total_eventCount : (total_eventCount + tracks[x].midiEvents.Length);
                total_notesPlayed = total_notesPlayed + tracks[x].notesPlayed;

                if (tracks[x].programs != null)
                {
            ...
            for (int x = 0; x < combinedTrackIdx+1; x++) {
                if (tracks[x].midiEvents == null) continue;
                        ...
                        case 0xFF:
                            ...
                            switch (statusByte) {
                                ...
                                case 0x01:
                                    ...
                                    int length = (int)ReadVarInt(tmp, ref index);
                                    MEv.Parameters[1] = UTF8Encoding.UTF8.GetString(tmp, index, length); index += length;
                                    break;
                                case 0x02:
        ...
        public ulong ReadVarInt(byte[] data, ref int index)
        {
            index--;
            ...
            while (index < data.Length)
            {
                ...
                if (++index == startIndex + 8 || (significantBitReached && (buffer & 0x80) != 0x80))
                    break;
            }
