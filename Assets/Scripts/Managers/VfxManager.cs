namespace TheOneStudio.HyperCasual.Managers
{
    ...
    public class VfxManager
    {
        ...
        private readonly IAudioService     soundService;

        public VfxManager(ObjectPoolManager objectPoolManager,
                          IScreenManager     screenManager,
                          IAudioService     soundService)
        {
        ...
        private async Task<GameObject> Spawn(string key) { return await this.objectPoolManager.Spawn(key); }

        public void SpawnVFXStar(Vector3 position) { this.SpawnVfx("EquipItemVfx", position); }
        ...
        public async void SpawnVfx(string key, Vector3 position, bool isLocal = false, Transform parent = null, float timeDelayToRecycle = 1f)
        {
            ...
            if (isLocal) vfx.transform.localPosition = position;
            else vfx.transform.position              = position;
        ...
        public async     void         SpawnNoAdsVfx() { this.SpawnVfx("vfx_no_ads", new Vector3(0,0,-.005f), parent: this.screenManager.CurrentActiveScreen.CurrentValue.CurrentTransform); }
        public async void SpawnUIVfx(string key, float timeDelayToRecycle = 2f, Transform parent = null)
        {
            ...
            else vfx.transform.SetParent(this.screenManager.CurrentRootScreen, worldPositionStays: false);
